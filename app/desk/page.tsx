...
export default async function Desk({searchParams}:{searchParams:Promise<{q?:string}>}){
  const{q=""}=await searchParams;const now=new Date();
  ...
  return <main className="fisk-desk"><header className="fisk-desk-nav"><Brand/><nav aria-label="Desk navigation"><Link className="active" href="/desk">Desk</Link><Link href="/watchlist">Watchlist</Link><Link href="/history">Research</Link><Link href="/journal">Activity</Link><Link href="/settings">Profile</Link></nav><AuthButton/></header><DeskWorkspace initialQuestion={q} news={news.items} newsError={news.error} stocks={stocks}/></main>;
}
