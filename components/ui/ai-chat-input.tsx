...
export function PromptInput({ value: controlled, onChange, onSubmit, placeholder = "Ask Fisk about the market…", className }: PromptInputProps) {
  ...
  const [effort, setEffort] = useState("Deep research");
  ...
  return <div className={cn("fisk-prompt", focused && "is-focused", value.length > 0 && "has-value", className)}>
    ...
    <textarea value={value} onChange={event=>update(event.target.value)} onKeyDown={keyDown} onFocus={()=>setFocused(true)} onBlur={()=>setFocused(false)} placeholder={placeholder} rows={2} aria-label="Ask Fisk"/>
    <div className="fisk-prompt-actions">
      <div><span className="model-chip"><i/> Fisk Qwen</span><button className="effort-chip" type="button" onClick={()=>setEffort(current=>current==="Deep research"?"Quick scan":"Deep research")}>{effort}</button></div>
      <div><button type="button" onClick={()=>inputRef.current?.click()} aria-label="Attach evidence"><Paperclip size={15}/></button><button className="prompt-send" type="button" onClick={submit} disabled={!value.trim()&&!attachments.length} aria-label="Start research"><ArrowUp size={16}/></button></div>
    </div>
  </div>;
}
