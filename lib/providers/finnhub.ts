...
export async function getCompanyNews(ticker:string,from:string,to:string) {
  ...
  try {
    const result=await cached(key,10*60_000,async()=>{
      const url=new URL("https://finnhub.io/api/v1/company-news"); url.searchParams.set("symbol",symbol);url.searchParams.set("from",from);url.searchParams.set("to",to);url.searchParams.set("token",requireEnv("FINNHUB_API_KEY"));
